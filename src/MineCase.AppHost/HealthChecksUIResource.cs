        ...
        foreach (var healthChecksUIResource in healthChecksUIResources)
        {
            foreach (var monitoredProject in healthChecksUIResource.MonitoredProjects)
            {
                ...
                if (!healthChecksEndpoint.Exists)
                {
                    ...
                    project.WithUrlForEndpoint(endpointName: monitoredProject.EndpointName, annotation =>
                    {
                        annotation.DisplayText = monitoredProject.EndpointName;
                        annotation.Url = monitoredProject.ProbePath;
                    });
